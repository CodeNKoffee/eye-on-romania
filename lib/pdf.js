...
export function exportChecklistPDF(filename, items, countryName = 'your country', isVisaFree = false) {
  ...
  items.forEach((item, index) => {
    ...
    // Add numbered item text with word wrapping (remove any existing numbering first)
    const cleanItem = item.replace(/^\d+\.\s*/, ''); // Remove any existing numbering
    const numberedText = `${index + 1}. ${cleanItem}`;
    const textLines = pdf.splitTextToSize(numberedText, 160);
