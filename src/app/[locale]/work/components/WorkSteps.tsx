      ...
      <div className="space-y-4 sm:space-y-6">
        {steps.map((step, index) => (
          <div key={index} className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            <div className={`flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 ${getStepStyles(step.color)} rounded-full flex items-center justify-center font-semibold text-sm sm:text-base self-start`}>
              {step.number}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex flex-col gap-2 mb-2">
                <h3 className="font-semibold text-transylvanian-stone text-base sm:text-lg leading-tight">{step.title}</h3>
              ...
              <p className="text-sm sm:text-base text-transylvanian-stone/70 leading-relaxed">{step.description}</p>
              {index < steps.length - 1 && (
                <div className="w-px bg-danube-mist h-4 sm:h-6 ml-4 sm:ml-6 mt-3 sm:mt-4 hidden sm:block" />
              )}
            </div>
          </div>
        ))}
