    ...
    <div className="bg-white rounded-xl shadow-sm border border-danube-mist p-4 sm:p-6 lg:p-8 mb-6 sm:mb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        {jobMarkets.map((market, index) => (
          <div key={index} className="flex flex-col sm:flex-row gap-3 sm:gap-4 p-3 sm:p-4 rounded-lg border border-danube-mist hover:shadow-sm active:shadow-none transition-shadow touch-manipulation">
            <div className={`flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 ${market.color} bg-current/10 rounded-lg flex items-center justify-center self-start`}>
              <div className={market.color}>
                {market.icon}
              </div>
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-transylvanian-stone mb-2 text-base sm:text-lg">{market.title}</h3>
              <p className="text-sm sm:text-base text-transylvanian-stone/70 mb-3 leading-relaxed">{market.description}</p>
              <div className="flex flex-col sm:grid sm:grid-cols-2 gap-2 text-xs">
                <div className="text-center sm:text-center p-2 bg-tricolor-blue/10 rounded">
                  <div className="font-medium text-tricolor-blue text-sm sm:text-base">{market.averageSalary}</div>
                  <div className="text-transylvanian-stone/60 text-xs">Avg. Salary</div>
                </div>
                <div className="text-center sm:text-center p-2 bg-carpathian-forest/10 rounded">
                  <div className="font-medium text-carpathian-forest text-sm sm:text-base">{market.demand}</div>
                  <div className="text-transylvanian-stone/60 text-xs">Demand</div>
                </div>
              </div>
            </div>
          </div>
