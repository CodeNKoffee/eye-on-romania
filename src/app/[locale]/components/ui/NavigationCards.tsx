...
export default function NavigationCards({ title, description, href, variant = 0 }: NavigationCardProps) {
  ...
  return (
    <Link
      href={href}
      className="group flex flex-col h-full p-8 bg-white rounded-xl shadow-sm border border-danube-mist hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1"
    >
      <Icon variant={variant} />
      <h3 className={`text-xl font-semibold text-transylvanian-stone mb-3 group-hover:${ctaColor} transition-colors`}>
        {title}
      </h3>
      <p className="text-transylvanian-stone/70 leading-relaxed flex-grow">
        {description}
      </p>
